class Api::V1::ChartsController < ApiController
      ...
      klasses.each do |klass|
        data = date_ranges.map do |range|
          ...
          unless params[:type] == '0'
            aggregate_data = aggregate_data.group((params[:category_size] || 'large') + '_category_id')
          end
  ...
  def amount
    ...
    expenses = Expense.joins(:period).calculating_target.where(periods: { year: params[:year],
                                                                          month: (1..prev_month) }).sum(:price)
    incomes = Income.joins(:period).calculating_target.where(periods: { year: params[:year],
                                                                        month: (1..prev_month) }).sum(:price)
    incomes - expenses
  end
end
